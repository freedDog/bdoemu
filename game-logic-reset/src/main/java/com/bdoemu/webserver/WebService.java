...
public class WebService implements Runnable{
    ...
    public WebService() {
        ...
        if (WebserverConfig.ENABLE) {
            ...
            ThreadPool.getInstance().executeGeneral(this);
        } else {
...
//                }
        ...
        Spark.get("/ErrorPage", (req, res) -> Spark.modelAndView( Collections.EMPTY_MAP, "ErrorPage"),  this.templateEngine);
    ...
    private void registerAdminPath() {
        Spark.get("/Admin/", (request, response) -> {
            ...
            return Spark.modelAndView(model.toMap(), "Areas/Admin/Index");
        },  this.templateEngine);
    }
    ...
    private void registerGuildCommentPath() {
        ...
        Spark.post("/guild/WriteGuildComment", (request, response) -> {
            ...
            final Guild guild = GuildService.getInstance().getGuild(model.getGuildNo());
            if (guild == null) {
                return Spark.modelAndView( Collections.EMPTY_MAP, "ErrorPage");
            }
            if (!guild.containsMember(model.getUserNo())) {
                return Spark.modelAndView( Collections.EMPTY_MAP, "ErrorPage");
            }
            guild.addComment(model.getUserNo(), model.getComment());
            return Spark.modelAndView( model.toMap(), "Areas/Guild/GuildComments");
        },  this.templateEngine);
        Spark.get("/guild/DeleteGuildComment", (request, response) -> {
            final GuildCommentViewModel model = new GuildCommentViewModel(request.queryMap().toMap());
            if (model.getCmtId() <= 0) {
                model.setParam("Invalid comment id.");
                return Spark.modelAndView( model.toMap(), "Areas/Guild/Error");
            }
            final Guild guild = GuildService.getInstance().getGuild(model.getGuildNo());
            if (guild == null) {
                return Spark.modelAndView(Collections.EMPTY_MAP, "ErrorPage");
        ...
        Spark.get("/guild/Error", (request, response) -> {
            ...
            return Spark.modelAndView( map, "Areas/Guild/Error");
        }, this.templateEngine);
    }
