...
public abstract class CreatureAI implements Runnable {
    ...
    private static Logger _log = LoggerFactory.getLogger(CreatureAI.class);

    /**
     * 所有全局注册的处理程序
     */
    private static Map<Long, Method> _handlers;
    /**
     * 通知当前AI已停止
     */
    private boolean _aiStopped;
    /**
     * 路径管理器的内部变量
     */
    private boolean _isFindPathCompleted;
    /**
     * 一个类被调用了多少次
     */
     ...
     * 脚本变量数组容器
     */
    private Map<Long, Integer[]> _aiVariableArray;
    /**
     * 当前正在执行的任务
     */
     ...
     * 将被执行的计划好的方法
     */
    private Consumer<Method> _scheduledMethod;
    /**
     * 拥有者
     */
     ...
     * 状态行为
     */
    protected EAIBehavior _behavior;
    /**
     * AI的开始时间
     */
    private Long _aiStartTime;
    ...
    /**
     * 当您需要根据目标的位置将字符移动到目标的左右时，可以使用这种方法。
     *
     * @param radius
     * @param naviType
     * @param onEnd
    ...
    /**
     * 当角色必须通过路由逻辑跟踪目标和他/她之间的距离时，就可以使用这种方法。
     *
    ...
    /**
     * 当你需要比较目标的距离和它自身的距离，并向目标的相反方向移动时，可以使用
     *
     * @param radius
     * @param onEnd
     * @param onExit
    ...
    /**
     * 当进入此状态时，生命值完全恢复，字符自动移动到初始衍生位置并处于移动状态。它处于不适合攻击的回避状态。
     *
     * @param onEnd
     * @param onExit
     */
    ...
    protected boolean findTargetInAggro(EAIFindTargetType findTargetType, EAIFindType findType, Predicate<Creature> filter) {
        ...
        switch (findTargetType) {
            case Enemy:
                targets = targets.filter(object -> getActor().isEnemy(object)).filter(filter);
                break;
            case EnemyLordOrKingTent:
                targets = targets.filter(Creature::isHousehold).filter(creatureObject -> getActor().isEnemy(creatureObject)).filter(filter);
                break;
            case EnemyBarricade:
                targets = targets.filter(Creature::isVehicle).filter(creatureObject -> creatureObject != getActor() && creatureObject.getTemplate().getVehicleType() == EVehicleType.RidableBarricade).filter(filter);
                break;
            case Corpse:
                targets = targets.filter(Creature::isDeadbody).filter(filter);
                break;
            case Player:
                targets = targets.filter(Creature::isPlayer).filter(filter);
                break;
            case PKPlayer:
                targets = targets.filter(creature -> creature.isPlayer() && ((Player) creature).getTendency() < 0).filter(filter);
                break;
            case Monster:
                targets = targets.filter(Creature::isMonster).filter(filter);
                break;
            case Npc:
                targets = targets.filter(Creature::isNpc).filter(filter);
                break;
            case Collect:
                targets = targets.filter(Creature::isCollect).filter(filter);
                break;
            case Character:
                targets = targets.filter(Creature::isCharacter).filter(filter);
                break;
            case AllyVehicle:
