...
public class PlayerSkillList extends JSONable {
    ...
    public PlayerSkillList(final Player owner) {
        ...
        this.awakeningSkills = new ConcurrentHashMap<>();
        this.owner = owner;
...
//            this.owner.sendPacket(new SMSetCharacterSkillPoints(this));
    ...
    public void applyPassiveBuffs() {
        this.skills.values().stream().filter(
                creatureSkill -> creatureSkill.getTemplate().getSkillType().isPassive()).forEach(
                creatureSkill -> SkillService.useSkill(this.owner, creatureSkill.getTemplate(), null, Collections.singletonList(this.owner)));
    }
