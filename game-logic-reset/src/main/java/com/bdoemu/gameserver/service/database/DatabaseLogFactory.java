...
public class DatabaseLogFactory {
    ...
    public void logItems(final Player player, final String operationType, final Collection<Item> items) {
        if (DatabaseConfig.ALLOW_DATABASE_LOGGING) {
            try {
                final List<DBObject> itemsDb = items.stream().map(item -> new BasicDBObject().append("date",
                        (int) (System.currentTimeMillis() / 1000L)).append("account_id", player.getAccountId()).append("operation_type", operationType).append("item_id",
                        item.getItemId()).append("count", item.getCount()).append("enchant_level", item.getEnchantLevel()).append("storage_location",
                        item.getStorageLocation().toString())).collect(Collectors.toList());
