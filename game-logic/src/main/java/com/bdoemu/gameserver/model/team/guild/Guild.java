...
public class Guild extends JSONable implements XMLRPCable {
    ...
    public Guild(final String name, final EGuildType guildType, final Player leader) {
        ...
        this.membersOnline = new ConcurrentHashMap<>();
        this.members = new ConcurrentHashMap<>();
        this._wars = new ConcurrentHashMap<>();
    ...
    public Collection<GuildQuestT> getAvailableQuests(final boolean isPreoccupancy) {
        return GuildQuestData.getInstance().getTemplates().stream().filter(
                g -> g.isPreoccupancy() == isPreoccupancy && g.getQuestLvl() == 1 && !GuildService.getInstance().containsQuest(g.getGuildQuestNr())
                        && (this.guildQuest == null || this.guildQuest.getQuestId() != g.getGuildQuestNr())).collect(Collectors.toList());
    }
