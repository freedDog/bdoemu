...
public class ServantSpawnInitData extends XmlDataHolder {
    ...
    private static Map<Integer, Map<Integer, Location>> table;
    ...
    public Location getSpawnLocation(final int npcId, final Servant servant) {
        ...
        ServantSpawnInitData.log.warn("Spawn placement for servants didn't exist in table (npcId={})!", (Object)npcId);
        return null;
    }

    static {
        log = LoggerFactory.getLogger((Class)ServantSpawnInitData.class);
        ServantSpawnInitData.table = new HashMap<Integer, Map<Integer, Location>>();
    }

    private static class Holder
    {
        static final ServantSpawnInitData INSTANCE;

        static {
            INSTANCE = new ServantSpawnInitData();
        }
    }
}
